...
app.get('/api/transfers/pending/:barcode', authenticateToken, requireActiveOrTrial, async (req: AuthRequest, res) => {
  try {
    ...
    if (!pendingTransfer) return res.status(400).json({ error: 'This item was not transferred to your branch' });
...
app.post('/api/transfers/receive/bulk', authenticateToken, requireActiveOrTrial, async (req: AuthRequest, res) => {
  try {
    const { barcodes } = req.body;
    const branchId = req.user!.branchId;
    if (!branchId) return res.status(400).json({ error: 'Please select a branch first' });
    if (!barcodes || !Array.isArray(barcodes)) return res.status(400).json({ error: 'Barcodes array is required' });

    const receivedItems = [];
    for (const barcode of barcodes) {
      const item = await prisma.item.findUnique({ where: { barcode_shopId: { barcode, shopId: req.user!.shopId! } } });
      if (!item || item.status !== 'In Transit') continue;
...
app.get('/api/admin/stats', authenticateToken, requireSuperAdmin, async (req: AuthRequest, res) => {
  try {
...
app.post('/api/sales/bulk', authenticateToken, requireActiveOrTrial, requireAccess([Role.OWNER, Role.MANAGER, Role.CASHIER], ['access_pos', 'delete_sale']), async (req: AuthRequest, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      const types = Array.from(new Set(itemsToSell.map((i: any) => i.type)));
      const rates = await tx.goldRate.findMany({
        where: { shopId, branchId: branchIdForRate, type: { in: types } }
      });
      
      const rateMap = new Map(rates.map((r: any) => [r.type, r.rate]));
      
      const missingRates = types.filter(t => !rateMap.has(t as string));
...
// --- Gold Rates ---
app.get('/api/gold_rates', authenticateToken, requireActiveOrTrial, async (req: AuthRequest, res) => {
  try {
    const shopId = req.user!.shopId!;
    let branchId = req.user!.branchId;
    if (!branchId) {
...
app.post('/api/gold_rates', authenticateToken, requireActiveOrTrial, requireAccess([Role.OWNER, Role.MANAGER], []), async (req: AuthRequest, res) => {
  try {
    const { type, rate } = req.body;
    const shopId = req.user!.shopId!;
    const branchId = req.user!.branchId;
    
    if (!branchId) return res.status(400).json({ error: 'Must be assigned to a branch to set rates' });
...
app.get('/api/cash_transfers', authenticateToken, requireActiveOrTrial, async (req: AuthRequest, res) => {
  try {
    const shopId = req.user!.shopId!;
...
app.put('/api/cash_transfers/:id/status', authenticateToken, requireActiveOrTrial, requireAccess([Role.OWNER, Role.MANAGER], ['view_vault']), async (req: AuthRequest, res) => {
  try {
...
app.put('/api/gold_transfers/:id/status', authenticateToken, requireActiveOrTrial, requireAccess([Role.OWNER, Role.MANAGER], ['view_vault']), async (req: AuthRequest, res) => {
  try {
    const id = req.params.id as string;
    const status = req.body.status as string; // 'ACCEPTED' or 'REJECTED'
    const shopId = req.user!.shopId!;
